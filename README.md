# Harper Learn

> A weekly video series documenting my journey learning Harper - from first steps to practical mastery.

<div align="center">

[![YouTube](https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://youtube.com/@your-channel)
[![Discord](https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/your-invite)
[![License](https://img.shields.io/badge/License-Apache%202.0-blue.svg?style=for-the-badge)](LICENSE)

**[Watch on YouTube](https://youtube.com/@your-channel)** • **[Join Discord Community](https://discord.gg/your-invite)**

</div>

---

## 📺 Weekly Episodes

Each week, I explore a new aspect of Harper through hands-on projects and real-world applications. Episodes are published every [day of week] at [time].

| Week | Topic | Status | Watch |
|------|-------|--------|-------|
| [Week 01](./week-01/) | Getting Started with Harper | ![Published](https://img.shields.io/badge/status-published-success) | [YouTube](./week-01/README.md) |
| [Week 02](./week-02/) | Installing from Source & First Static Site | ![Planned](https://img.shields.io/badge/status-planned-blue) | — |
| [Week 03](./week-03/) | Local vs Remote Deployments & Build Pipelines | ![Planned](https://img.shields.io/badge/status-planned-blue) | — |
| [Week 04](./week-04/) | TBD | ![Planned](https://img.shields.io/badge/status-planned-blue) | — |

> **Note:** This series is ongoing! New episodes are added regularly based on community feedback and my learning progression.

---

## 💬 Join the Community

Learning is better together! Join our **[Discord community](https://discord.gg/your-invite)** to:

- 🎯 Ask questions and get real-time help
- 🤝 Connect with other Harper learners
- 💡 Share your own projects and insights
- 🗣️ Participate in weekly discussions about episodes
- 🎉 Celebrate wins and troubleshoot challenges together

The Discord is our conversational hub—come say hello!

---

## 🎯 Request Topics & Share Feedback

Have an idea for a future episode? Want to share feedback on what you've watched?

- **[Request a Topic](../../issues/new?template=topic-request.yml)** — Suggest topics you'd like to see covered
- **[Share Feedback](../../issues/new?template=feedback.yml)** — Let me know what's working and what could be better

Browse all suggestions and vote on topics you'd like to see by checking out the **[Issues](../../issues)** page.

> **Tip:** For quick questions or casual discussions, head to [Discord](https://discord.gg/your-invite). For structured feedback and topic requests, use GitHub Issues.

---

## 📖 About This Repository

This repository serves as the companion resource for the Harper Learn video series. Each week's directory contains:

- **README.md** — Episode summary, talk track, timestamps, and links to watch on various platforms
- **Code samples** — Practical examples demonstrated in the video (when applicable)

The goal is to create a living learning resource that grows alongside the series, making it easy to reference past content and track the learning journey.

---

## 📜 License

This project is licensed under the Apache License 2.0 - see the LICENSE file for details.

---

<div align="center">

**Happy Learning!** 🚀

*Don't forget to subscribe and join the Discord for the full experience.*

</div>
